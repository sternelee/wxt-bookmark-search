export default {
  ...
  options: {
    ...
    search: {
      title: "🔍 検索戦略",
      mode: "検索モード",
      modeHint: "ハイブリッド検索はキーワードの精度と AI の意味理解を組み合わせます",
      modeHybrid: "ハイブリッド検索 (推奨)",
      modeVector: "ベクトル検索のみ",
      modeKeyword: "キーワード一致 (クラシック)",
      vectorWeight: "ベクトル検索ウェイト",
      vectorWeightHint: "値を上げると意味的な類似性が重視されます。下げると文字列一致が重視されます",
      applied: "✓ 検索設定を適用しました",
    },

    github: {
      title: "🐙 GitHub Stars セマンティックインデックス",
      tokenHint: "GitHub Stars の同期には starred repos の読み取りが必要です。下記の Gist ブックマーク同期を有効にする場合は gist 権限も必要です。",
      saveSettings: "GitHub 設定を保存",
      syncStars: "Stars を今すぐ同期",
      syncingStars: "Stars を取得中...",
      syncSuccess: "✓ 同期成功！{{total}} 個のリポジトリをインデックスキューに追加しました",
    ...
    twitter: {
      ...
      description: "ブラウザで Twitter/X にログインしている必要があります。拡張機能は自動的に Cookie を抽出して同期します。開発者アカウントは不要です。",
      enableSync: "Twitter ブックマーク同期を有効化",
      csrfToken: "CSRF Token (ct0) - 任意",
      csrfPlaceholder: "自動抽出失敗時に手動入力",
      csrfHint: "自動抽出に失敗した場合、ブラウザの開発者ツールからコピーできます",
      ...
      syncSuccess: "✓ 同期成功！{{total}} 個のブックマークをインデックスに追加しました",
    ...
    history: {
      ...
      description: "ブラウザの閲覧履歴をセマンティック検索に含めます。http/https ページのみをインデックスし、既存のブックマーク/GitHub/Twitter 記録はスキップします。",
      enableSync: "ブラウジング履歴同期を有効化",
      syncDays: "最近 N 日間を同期",
      syncDaysHint: "範囲 1-365 日、デフォルト 30 日。日数が多いほど初回同期は遅くなります。",
      saveSettings: "設定を保存",
      syncNow: "履歴を今すぐ同期",
      syncingNow: "同期中...",
    ...
    gist: {
      ...
      description: "GitHub Gist を介して複数デバイス間でブックマークを同期します。上記で gist 権限を持つ GitHub Token を設定する必要があります。",
      ...
      confirmUploadBody: "ローカルブックマークで Gist の内容を全置換します。リモートのみのブックマークは失われます。この操作は元に戻せません。",
      confirmDownloadTitle: "⬇️ ダウンロード上書きの確認",
      confirmDownloadBody: "Gist の内容でローカルブックマークを全置換します。ローカルのみのブックマークは削除されます。この操作は元に戻せません。",
      creating: "Gist を作成してブックマークをアップロード中...",
      createSuccess: "✓ Gist 作成成功！ID: {{gistId}}",
      linking: "Gist を検証して連携中...",
      linkSuccess: "✓ 連携成功！Gist ID: {{gistId}}。「今すぐ同期」をクリックして同期を開始してください。",
      syncInfo: "ブックマークを Gist に同期中...",
      syncSuccess: "✓ 同期完了！+{{added}} 追加, -{{removed}} 削除, {{uploaded}} 合計",
      uploadInfo: "ローカルブックマークを Gist にアップロードして上書き中...",
      uploadSuccess: "✓ アップロード完了！{{uploaded}} 件のブックマークを Gist に上書きしました",
      downloadInfo: "Gist をダウンロードしてローカルブックマークを上書き中...",
      downloadSuccess: "✓ ダウンロード完了！+{{added}} 件のブックマークを復元、{{removed}} 件のローカル項目を削除しました",
      ...
      noGistLinked: "Gist が連携されていません。先に作成または連携してください。",
      sizeError: "Chrome ブックマークマネージャーで不要なブックマークを整理してから再同期するか、「アップロードで上書き」を使用して新しい Gist を作成してください。",
    ...
    indexManager: {
      ...
      scopeHint: "フォルダを選択しない場合、デフォルトですべてのブックマークを増分インデックスします",
      ...
      folderSynced: "✓ 選択したフォルダ ({{total}} 個のブックマーク) を完全に同期しました",
    ...
    failedBookmarks: {
      ...
      description: "これらのブックマークはインデックス処理中にエラーが発生しました。URL が無効になっている可能性があります。リンクをクリックしてテストするか、直接ブックマークを削除してください。",
      noTitle: "タイトルなし",
      visit: "クリックしてアクセス",
      errorLabel: "エラー",
      deleteConfirm: "ブラウザからこのブックマークを完全に削除してもよろしいですか？",
  ...
  search: {
    ...
    tryOther: "他のキーワードを試すか、/github /twitter でソースを絞り込んでください",
  ...
  background: {
    omniboxDefault: "🔍 Flow Search — キーワードを入力して Enter で全ページ検索を開くか、ブックマークを選択してジャンプ (/github /twitter /folder:名前)",
